...
<body>
    ...
    <?php 
        ...
        $res=mysqli_query($sum,"select * from etudiant order by nom_etudiant,num_filiere,niveau_etudiant");
        if($_POST)
        {
            $numero=$_POST['numero'];
            $nom=$_POST['nom'];
            if(!empty($numero) || !empty($nom))
            {
                if(!empty($nom))
                {
                    $resultat=mysqli_query($sum,"DELETE FROM etudiant WHERE numero_etudiant=$nom");
                }
                else if(!empty($numero))
                {
                    $resultat=mysqli_query($sum,"DELETE FROM etudiant WHERE numero_etudiant=$numero");
                }
                if($resultat==0)
                {
                    echo '
                    <div class="alert">
                        ...
                        <div>
                            ...
                            <h3>UNE ERREUR S\'EST PRODUIT'.$nom.'</h3>
                            <h3>VEUILEZ RESSAYER ULTERIEUREMENT.</h3>
                            <label for="btn-alert"><a href="supprimerEtudiants.php">FERMER</a></label>
                        </div>
                    </div>
                    ';
                }
                else
                {
                    ...
                    <div class="alert">
                        ...
                        <div>
                            ...
                            <h3>L\' etudiant  est supprimé avec succes de la base</h3>
                            <h3>Merci</h3>
                            <label for="btn-alert"><a href="supprimerEtudiants.php">FERMER</a></label>
                        </div>
                    </div>
    ...
    <section id="corps">
        <form action="" method="POST">
            ...
            <small>VEUILEZ ENTRER SOIT LE NUMERO DE L'ETUDIANT, SOIT LE NOM DE L'ETUDIANT</small>
            <span>Numéro: </span>
            <span>Nom: </span>
            <input type="number" name="numero" placeholder="exemple: 0200004321">
            <select name="nom">
                <option value=""></option>
